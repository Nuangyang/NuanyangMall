...
$(function () {
    ...
    //获取所有商品信息
    $.post("/goods.do?method=getGoodsShow",function (dom) {
        ...
        var pagesd=1;
        //存储当前页数
        sessionStorage.setItem("userpage",pagesd);
        ...
        //判断是否在主页有做搜索操作
       $.post("/goods.do?method=UserByIfVague",function (domsc) {
            if(domsc.code==200){
                //渲染用户主页搜索内容
                for(var i=0;i<=11;i++) {
                    var str = "  <div class=\"goods-item\" gid='" + domsc.data[i].gid + "'>\n" +
                        "<a href=\"javascript:void(0)\" class=\"figure\" onclick='getgoodsdetails(this)' gid='"+dom.data[i].gid+"'>\n" +
                        "<div class=\"figure-imgs\"><img src=\"" + domsc.data[i].img + "\" alt=\"\"></div>\n" +
                        "<h2 class=\"title\">" + domsc.data[i].gname + "</h2>\n" +
                        "<p class=\"prive\" >\n" +
                        "<span>" + domsc.data[i].gprive + "</span>\n" +
                        "<span>元起</span>\n" +
                        "</p>\n" +
                        "<div class=\"thumbs\"><img src=\"" + domsc.data[i].img + "\" alt=\"\"></div>\n" +
                        "</a>\n" +
                        "</div>"
                    $(".goods-list-clearfix").append(str);

                    //渲染完成后删除session
                    $.post("/goods.do?method=delGoodsVague",function (dom) {
                ...
                //动态构建商品信息
                for(var i=0;i<=11;i++){
                    var str="  <div class=\"goods-item\" gid='"+dom.data[i].gid+"'>\n" +
                        "<a href=\"javascript:void(0)\" class=\"figure\" onclick='getgoodsdetails(this)' gid='"+dom.data[i].gid+"'>\n" +
                        "<div class=\"figure-imgs\"><img src=\""+dom.data[i].img+"\" alt=\"\"></div>\n" +
                        "<h2 class=\"title\">"+dom.data[i].gname+"</h2>\n" +
                        "<p class=\"prive\" >\n" +
                        "<span>"+dom.data[i].gprive+"</span>\n" +
                        "<span>元起</span>\n" +
                        "</p>\n" +
                        "<div class=\"thumbs\"><img src=\""+dom.data[i].img+"\" alt=\"\"></div>\n" +
                        "</a>\n" +
                        "</div>"
                    $(".goods-list-clearfix").append(str);
        ...
        //动态构建底部分页
        $.post("/goods.do?method=getFenyedao",function (doms) {
            console.log(doms);
           var goodspagest=doms.data.pagest
            for (var is=1;is<=goodspagest;is++){
                var sla= "<a href=\"javascript:void(0);\" onclick=\"add01(this)\" pagennum=\""+is+"\">"+is+"</a>\n";
                $("#daohang").append(sla);
            }
                var cs="<a href=\"javascript:void(0);\" onclick=\"sXye(this)\"pagennum=\"-2\">下一页</a>\n"
                $("#daohang").append(cs)
        },"json")
    },"json")
})
...
function add01(dos) {
    ...
    //根据用户点击对应页数 更新商品
    var anniu=$(dos)
    $.post("/goods.do?method=getPageShow",{pages:anniu.attr("pagennum")},function (dom) {
        //清除现有的展示商品
        $(".goods-list-clearfix").empty();
     ...
     /*存取session*/
       /* sessionStorage.setItem()
        ...
        /*存储当前页数*/
       sessionStorage.setItem("userpage",anniu.attr("pagennum"));
...
function sXye(dom) {
 var user=$(dom)
//-1上一页  //-2是下一页
   var num=user.attr("pagennum");
    if(num==-1){
    //上一页
         var numspage=sessionStorage.getItem("userpage");
         ...
         if(numspage==1){
             ...
             $.post("/goods.do?method=getPageShow",{pages:numspage},function (dom) {
                 //清除现有的展示商品
                 $(".goods-list-clearfix").empty();
                 //渲染商品
                 for(var i=0;i<dom.data.length;i++){
                 ...
                 /*存储当前页数*/
                 sessionStorage.setItem("userpage",numspage);
...
//模糊查询
function vaguues(stru) {
    //关闭表单默认提交
    event.preventDefault();
    
    console.log($(".search-text").val())
    
    $.post("/goods.do?method=getVagueGoods",{vague:$(".search-text").val()},function (dom) {
        //删除当前商品页面
        $(".goods-list-clearfix").empty();
        //判断是否传回数据
...
//分类查询
function category(dss){
    //获取用户点击
    var ca=$(dss)
    var cate=ca.attr("caid")
    
    $.post("/goods.do?method=categoryShowGoods",{category:cate},function (doue) {
        ...
        if(doue.code==200){
            ...
            //渲染商品
            for(var i=0;i<=11;i++){
                var str = "  <div class=\"goods-item\" gid='" + doue.data[i].gid + "'>\n" +
                    "<a href=\"javascript:void(0)\" class=\"figure\" onclick='getgoodsdetails(this)' gid='"+doue.data[i].gid+"'>\n" +
                    "<div class=\"figure-imgs\"><img src=\"" + doue.data[i].img + "\" alt=\"\"></div>\n" +
                    "<h2 class=\"title\">" + doue.data[i].gname + "</h2>\n" +
                    "<p class=\"prive\" >\n" +
                    "<span>" + doue.data[i].gprive + "</span>\n" +
                    "<span>元起</span>\n" +
                    "</p>\n" +
                    "<div class=\"thumbs\"><img src=\"" + doue.data[i].img + "\" alt=\"\"></div>\n" +
                    "</a>\n" +
                    "</div>"
                $(".goods-list-clearfix").append(str);
...
*/
   ...
   */
    ...
    console.log(snum)
    if(snum==0||snum==undefined){
        sto.style.background="#ff6700"
        snum=snum++;
    }else{
        sto.style.background="#fff"
        snum=0;
    }
...
function getgoodsdetails(dox){
    ...
    $.post("/goods.do?method=getUserByGoodsID",{gid:gids},function (goodsid) {
        ...
        //存入session
        sessionStorage.setItem("showsid",goods)
