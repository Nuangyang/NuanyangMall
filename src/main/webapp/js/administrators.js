...
$(function () {
    ...
    //预加载商品管理界面
        var str="    <table border=\"1px\" cellspacing=\"0px\" bordercolor=\"red\" align=\"center\" style='margin: -50px 0px 0px 0px'>\n" +
            "        <tr>\n" +
            "        <td colspan=\"7\"><h3>新增</h3></td>\n" +
            "    </tr>\n" +
            "    <tr>\n" +
            "    <td>商品名称</td>\n" +
            "    <td>商品价格</td>\n" +
            ...
            "        <td><input type=\"text\" name=\"stock\" id=\"gnum\" value=\"\" /></td>\n" +
            "        <td><input type=\"text\" name=\"address\" id=\"jianji\" value=\"\" /></td>\n" +
            ...
            "        </select>\n" +
            "        </td>\n" +
            "        <td><button id=\"upload\" onclick='addGoodsadmini()'>提交</button></td>\n" +
            "    </tr>\n" +
            "    <tr>\n" +
            "    <td colspan=\"4\"><h3>下架商品</h3></td>\n" +
            "    <td colspan=\"3\"><input type=\"text\" placeholder=\"搜索商品\" class='sougoods'/>\n" +
            "        <button class=\"sousuo\" onclick='sousuo(this)'>搜索</button></td>\n" +
            "\n" +
            "    </tr>\n" +
            "    <tr>\n" +
            "    <td>名称</td>\n" +
    ...
    //获取所有商品类别信息
    $.post("/admin.do?method=getGoodsAdminist",function (dom) {
        if(dom.code==200){
            for(var i=0;i<dom.data.length;i++){
                var str="<option value='"+dom.data[i].caid+"'>"+dom.data[i].cname+"</option>"
    ...
    //获取所有商品信息
    $.post("/admin.do?method=getGoodsAdministAll",function (dogoods) {
    console.log(dogoods)

        if(dogoods.code==200){
            for(var i=0;i<dogoods.data.length;i++){
                str="    <tr>\n" +
                    "                       <td>"+dogoods.data[i].gname+"</td>\n" +
                    "                       <td ><img src=\""+dogoods.data[i].img+"\" alt=\"\"  class=\"goodsimg\"/></td>\n" +
                "                       <td><span class=\"snum\">"+dogoods.data[i].gnum+"</span></td>\n" +
                "                       <td><span>"+dogoods.data[i].gprive+"</span></td>\n" +
                "                       <td colspan=\"3\">\n" +
                "                           <form action=\"/admin.do?method=xiugaiGoods&gid="+dogoods.data[i].gid+"\" method=\"post\" class=\"for-goods\">\n" +
                "                               <input type=\"\" placeholder=\"修改商品名\" name='gnames'/>\n" +
                "                               <input type=\"\" placeholder=\"修改商品库存\" name='gnums'/>\n" +
                "                               <input type=\"\" placeholder=\"修改价格\" name='gprives' />\n" +
                "                               <br />\n" +
                "                               <input type=\"submit\" value=\"确认修改\"/>\n" +
                "                           </form>\n" +
                "\n" +
                "                           <button class=\"del\" onclick=\'xiajia(this)\' gid='"+dogoods.data[i].gid+"'>删除\n" +
                "                           </button></td>\n" +
                "                   </tr>"
...
function usermanage() {
    ...
    //加载用户管理界面
    var str=" <table border=\"1px\" cellspacing=\"0px\" bordercolor=\"red\" align=\"center\" class=\"user\" >\n" +
        "                <div>\n" +
        "                    <tr >\n" +
        "                        <td colspan=\"4\">\n" +
        "                            <h3>用户名</h3>\n" +
    ...
    //读取所有用户信息
    $.post("/admin.do?method=getUserAdministAll",function (douser) {
      console.log(douser)
        if(douser.code==200){
            for(var i=0;i<douser.data.length;i++){
                var st;
                if(douser.data[i].state==4){
                    st="已被禁用"
                }else{
                    st="正常"
                }
                var  str="<tr>\\n\" +\n" +
                    "<td colspan=\"4\">"+douser.data[i].uname+"</td>\n" +
                    "<td>"+douser.data[i].uemail+"</td>\n" +
                    "<td>"+douser.data[i].birthday+"</td>\n" +
                    "<td>"+douser.data[i].uphone+"</td>\n" +
                    "<td><span>"+st+"</span></td>\n" +
                    "<td><button class=\"user-button\" uid='"+douser.data[i].uid+"' onclick='jiejing(this)'>恢复</button>\n" +
                    "<button class=\"user-button\"style=\"background-color: #c7bdbb;\" uid='"+douser.data[i].uid+"' onclick='jinyong(this)'>禁用</button>\n" +
                    "<p>-</p>\n" +
                    "</td>\n" +
                    "</tr>"
...
function sousuo() {
    //获取搜索框输入
    var shousuo=$(".sougoods").val()
    if(null!=shousuo){
        //发送后端请求数据
        $.post("/goods.do?method=getVagueGoods",{vague:shousuo},function (doms) {
            if(doms.code==200){
                //清除当前商品信息
                $("#tb").empty();
                //渲染查询出来得商品
                for(var i=0;i<doms.data.length;i++) {
                    str = "    <tr>\n" +
                        "                       <td>" + doms.data[i].gname + "</td>\n" +
                        "                       <td ><img src=\"" + doms.data[i].img + "\" alt=\"\"  class=\"goodsimg\"/></td>\n" +
                        "                       <td><span class=\"snum\">" + doms.data[i].gnum + "</span></td>\n" +
                        "                       <td><span>" + doms.data[i].gprive + "</span></td>\n" +
                        "                       <td colspan=\"3\">\n" +
                        "                           <form action=\"/admin.do?method=xiugaiGoods&gid="+doms.data[i].gid+"\" method=\"post\" class=\"for-goods\">\n" +
                        "                               <input type=\"\" placeholder=\"修改商品名\" name='gnames'/>\n" +
                        "                               <input type=\"\" placeholder=\"修改商品库存\" name='gnums'/>\n" +
                        "                               <input type=\"\" placeholder=\"修改价格\" name='gprives'/>\n" +
                        "                               <br />\n" +
                        ...
                        "                           </form>\n" +
                        "\n" +
                        "                           <button class=\"del\" onclick=\'xiajia(this)\' gid='"+doms.data[i].gid+"'>删除\n" +
                        "                           </button></td>\n" +
                        "                   </tr>"
                ...
                alert("哦豁~没有这个商品 怎么办呢 你是管理员不会自己添加？")
            }
        },"json")
    }else{
        alert("你虽然是管理员但是也不能空着搜索吧")
    }
}
...
//新增商品
function addGoodsadmini() {
    //获取商品类别
    var goodsleibie=$("#category").val()
    //获取商品名称
    ...
    var gnum=$("#gnum").val()
    //获取商品简介
    var jianji=$("#jianji").val()
    //获取商品图片
    var formData = new FormData();
    if(null!=goodsleibie&&null!=goodsname&&null!=gprice&&null!=gnum&&null!=jianji){
        formData.append("myfile", document.getElementById("file").files[0]);
        formData.append("gname",goodsname );
        formData.append("gprice",gprice );
        formData.append("gnum",gnum);
        formData.append("jianji",jianji);
        formData.append("goodsleibie",goodsleibie );

        $.ajax({
            url: "http://localhost/admin.do?method=addGoodsAdminist",
            type: "POST",
            data: formData,
            /**
...
function xiajia(dom) {
    ...
    $.post("/admin.do?method=delGoodsAdminist",{gid:gid},function(doss) {
...
//禁用用户
function jinyong(dom) {

   var uid= $(dom).attr("uid")

    $.post("/admin.do?method=jingYong",{uid:uid},function (doms) {
        if(doms.code==200){
            alert("禁用成功！打入小黑屋")
            //刷新页面
            location.reload();
        }else{
            alert("关进小黑屋的人 可不能在关进小黑屋了哦")
        }
    },"json")
}

//解禁用户
function jiejing(dom) {
    var uid= $(dom).attr("uid")

    $.post("/admin.do?method=jieJing",{uid:uid},function (doms) {
        if(doms.code==200){
            alert("解开成功！！让他别再进来了")
            //刷新页面
            location.reload();
        }else{
            alert("他已经出去了不要在解禁了 你个管理员没脑子？")
        }
    },"json")
}
...
function updatePassword(){
    ...
    //获取所有的管理员
    $.post("/admin.do?method=getAdminist",function (dso) {
        console.log(dso)
        if (dso.code==200){


            for(var i=0;i<dso.data.length;i++){
                var st;
                if(dso.data[i].jurisdiction==1){
                    st="全级管理员"

                }else{
                    st="管理"
                }
                var  str="<tr><td colspan=\"4\">"+dso.data[i].bsid+"</td><td>"+dso.data[i].adname+"</td><td><span>"+st+"</span></td>\n" +
                    "<td><input type=\"text\" placeholder=\"旧密码\" class='jpwd'/></td>\n" +
                    "<td><input type=\"text\" placeholder=\"新密码\" class='spwd'/></td>\n" +
                    "<td><button class=\"user-button\" aid='"+dso.data[i].bsid+"' onclick='xiugeaipwd(this)' >修改密码</button>\n" +
                    "<p>-</p>\n" +
                    "</td>\n" +
                    "</tr>"
...
function xiugeaipwd(dom) {
    ...
    if(null!=jpwd && null!=xpwd){
            ...
            if(jpwd!=xpwd){
                //发送请求判断旧密码是否相同
                $.post("/admin.do?method=updataAdministPwd",{jpwd:jpwd,xpwd:xpwd,id:id},function (doms) {
                        if (doms.code==200){
                            alert("修改成功")
                            //刷新界面
                            location.reload();
...
//点击订单管理
function dindanguanli() {

    //清空右边表格
    $("#right").empty();

    //加载订单管理界面
    var str=" <table border=\"1px\" cellspacing=\"0px\" bordercolor=\"red\" align=\"center\" class=\"user11\">\n" +
        "                <div>\n" +
        "                    <tr >\n" +
        "                        <td colspan=\"\">\n" +
        "                            <h3>用户ID</h3>\n" +
    ...
    //获取所有的订单信息
    $.post("/admin.do?method=getUserAllAdminist",function (dom) {

        console.log(dom)

        if(dom.code==200){
            for(var is=0;is<dom.data.length;is++){

                //拿到订单时间
                var  timdedetail=dom.data[is].creationtime;//假如是10位毫秒需要乘1000
                ...
                if(dom.data[is].dinstart==2){

                    var str="<tr>\\n\" +\n" +
                        "<td colspan=\\\"3\\\">"+dom.data[is].uid+"</td>" +
                        "<td>"+dom.data[is].uuid+"</td>" +
                        "<td>"+dom.data[is].address+"</td>" +
                        "<td><span>"+time+"</span></td>" +
                        "<td>" +
                        "<button class=\\\"user-button\\\">已支付</button>" +
                        "<p>-</p>" +
                        "</td>" +
                        "</tr>";
                    ...
                    var str="<tr>" +
                        ...
                        "<td><span>"+time+"</span></td>" +
                        "<td>" +
                        "<button class=\"user-button\"style=\"background-color: #c7bdbb;\">未支付</button>" +
                        "<p>-</p>" +
                        "</td>" +
                        "</tr>";
                    $(".user11").append(str)
