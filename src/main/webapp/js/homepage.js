...
$(function () {
    ...
    //获取所有主页商品
    $.post("/goods.do?method=getOnlineGoods",function (dom) {
        console.log(dom)

        //判断是否登录
            $.post("/user.do?method=signinHomepage",function (dom) {
                //判断管理员与普通用户
                /*登录成功*/
                //预加载商品
                if(dom.code==200){
                    ...
                    $(".hometools").empty()
                    //获取用户基本信息
                    $.post("/user.do?method=getUserByzhuye",{"user":dom.data.uname},function (doms) {

                        console.log(doms)

                        if(doms.data.head==null||doms.data.head==""){
                            //加载默认头像
                            var utr="<a href=\"PersonalCenter.html\" class=\"dateva\">\n" +
                                "\t\t<div class=\"icon\">\n" +

                                "\t\t\t<img src=\"../../images/defuimg.png\" alt=\"\" class=\"default\">\n" +
                                "\t\t</div>\n" +
                                "\t\t<span class=\"datev\">"+doms.data.uname+"</span>\n" +
                                "\n" +
                                "\t</a>"

                            var utr1="<a href=\"gouwuche.html\" class=\"dateva\">\n" +
                                "\t\t\t<div class=\"icon\">\n" +
                                "\t\t\t\t<img src=\"../../images/gouwuche.png\" alt=\"\" class=\"default\">\n" +
                                "\t\t\t</div>\n" +
                                "\t\t\t<span class=\"datev\">购物车</span>\n" +
                                "\n" +
                                "\t\t</a>"



                            $(".hometools").append(utr)
                            $(".hometools").append(utr1)
                            ...
                            var utr="<a href=\"PersonalCenter.html\" class=\"dateva\">\n" +
                                ...
                                "\t\t\t<img src=\""+doms.data.head+"\" alt=\"\" class=\"default\">\n" +
                                "\t\t</div>\n" +
                                "\t\t<span class=\"datev\">"+doms.data.uname+"</span>\n" +
                                "\n" +
                                "\t</a>"
                        ...
                        if(doms.data.state==1){
                            //激活账户
                            var dsa="\t<a href=\"javascript:void(0)\" class=\"denglv\">欢迎您亲爱的</a>\n" +
                                "\t\t\t\t\t\n" +
                                "\t\t\t\t\t\t\t\t<a href=\"javascript:void(0)\" class=\"zhuce\" style=\"color: #ffec38\">S VIP</a>"
                            $(".topbar-info").append(dsa);
                            var utr2="\t\t<a href='javascript:void(0)' class=\"deltui\" onclick=\"delUser()\">退出登录</a>"
                            $(".topbar-info").append(utr2);

                        }else{
                            //没有激活账户
                            var ds= "\t\t\t\t\t\t\t\t<a href=\"javascript:void(0)\" class=\"denglv\">欢迎您亲爱的</a>\n" +
                                "\t\t\t\t\t\n" +
                                "\t\t\t\t\t\t\t\t<a href=\"Activate.html\" class=\"zhuce\" style=\"color: red\">注册会员</a>"
                            $(".topbar-info").append(ds);
                    ...
                    //修改顶部条状态
                    $(".topbar-info").empty()
                    var svip="<a href=\"Administratorinterface.html\" class=\"dateva\">\n" +
                        "\t\t\t<div class=\"icon\">\n" +

                        "\t\t\t\t<img src=\"../../images/asi.png\" alt=\"\" class=\"default\">\n" +
                        "\t\t\t</div>\n" +
                        "\t\t\t<span class=\"datev\" style=\"color: red\">管理平台</span>\n" +
                        "\n" +
                        "\t\t</a>"

                    $(".hometools").append(svip)


                    //激活账户
                    var dsa="\t<a href=\"javascript:void(0)\" class=\"denglv\">辛苦啦！</a>\n" +
                        "\t\t\t\t\t\n" +
                        "\t\t\t\t\t\t\t\t<a href=\"javascript:void(0)\" class=\"zhuce\" style=\"color: #ffec38\">超级管理员</a>"
                    $(".topbar-info").append(dsa);
                    var utr2="\t\t<a href='javascript:void(0)'  class=\"deltui\" onclick=\"delUser()\">退出登录</a>"
                    $(".topbar-info").append(utr2);
        ...
        //加载排行榜======================
        //排行榜修改ID 6.4.3.7.8.5.9 从小到大
        var str="<li><a href=\"javascript:void(0)\" class=\"to1\" gid='"+dom.data[6].gid+"' onclick='getgoodsdetails(this)'><i class=\"paihang_i\" >1</i>\n" +
            "\t\t\t\t\t\t<span>"+dom.data[6].gname+"</span>\n" +
            "\t\t\t\t\t</a></li>\n" +
            "\t\t\t\t\t\t<li><a href=\"javascript:void(0)\" class=\"to2\" gid='"+dom.data[4].gid+"' onclick='getgoodsdetails(this)'><i class=\"paihang_i\">2</i>\n" +
            "\t\t\t\t\t\t\t<span>"+dom.data[4].gname+"</span>\n" +
            "\t\t\t\t\t\t</a></li>\n" +
            "\t\t\t\t\t\t<li><a href=\"javascript:void(0)\" class=\"to3\" gid='"+dom.data[3].gid+"' onclick='getgoodsdetails(this)'><i class=\"paihang_i\">3</i>\n" +
            "\t\t\t\t\t\t\t<span>"+dom.data[3].gname+"</span>\n" +
            "\t\t\t\t\t\t</a></li>\n" +
            "\t\t\t\t\t\t<li><a href=\"javascript:void(0)\" gid='"+dom.data[7].gid+"' onclick='getgoodsdetails(this)'><i class=\"paihang_i\">4</i>\n" +
            "\t\t\t\t\t\t\t<span>"+dom.data[7].gname+"</span>\n" +
            "\t\t\t\t\t\t</a></li>\n" +
            "\t\t\t\t\t\t<li><a href=\"javascript:void(0)\" gid='"+dom.data[8].gid+"' onclick='getgoodsdetails(this)'><i class=\"paihang_i\">5</i>\n" +
            "\t\t\t\t\t\t\t<span>"+dom.data[8].gname+"</span>\n" +
            "\t\t\t\t\t\t</a></li>\n" +
            "\t\t\t\t\t\t<li><a href=\"javascript:void(0)\" gid='"+dom.data[5].gid+"' onclick='getgoodsdetails(this)'><i class=\"paihang_i\">6</i>\n" +
            "\t\t\t\t\t\t\t<span>"+dom.data[5].gname+"</span>\n" +
            "\t\t\t\t\t\t</a></li>\n" +
            "\t\t\t\t\t\t<li><a href=\"javascript:void(0)\" gid='"+dom.data[9].gid+"' onclick='getgoodsdetails(this)'><i class=\"paihang_i\">7</i>\n" +
            "\t\t\t\t\t\t\t<span>"+dom.data[9].gname+"</span>\n" +
            "\t\t\t\t\t\t</a></li>"
            $(".paihangul").append(str);

        //加载主页分类条=====================
        //分类条修改 6.4.3.7.8从小到大
        var stt="\t<li onclick='addvido(this)' video='"+dom.data[6].video+"'><a href=\"javascript:void(0)\" gid='"+dom.data[6].gid+"'>"+dom.data[6].gname+"</a></li>\n" +
            "\t\t\t\t\t\t<li onclick='addvido(this)' video='"+dom.data[4].video+"'><a href=\"javascript:void(0)\" gid='"+dom.data[4].gid+"'>"+dom.data[4].gname+"</a></li>\n" +
            "\t\t\t\t\t\t<li onclick='addvido(this)' video='"+dom.data[3].video+"'><a href=\"javascript:void(0)\" gid='"+dom.data[3].gid+"'>"+dom.data[3].gname+"</a></li>\n" +
            "\t\t\t\t\t\t<li  onclick='addvido(this)' video='"+dom.data[7].video+"'><a href=\"javascript:void(0)\" gid='"+dom.data[7].gid+"'>"+dom.data[7].gname+"</a></li>\n" +
            "\t\t\t\t\t\t<li onclick='addvido(this)' video='"+dom.data[8].video+"'><a href=\"javascript:void(0)\"  gid='"+dom.data[8].gid+"'>"+dom.data[8].gname+"</a></li>"
            $(".fenleiul").append(stt);
        ...
        for(var i=13;i<=22;i++){
            var sou="<li class=\"brick-item\">\n" +
                "\t\t\t\t\t\t\t\t\t\t<a href=\"javascript:void(0)\" gid='"+dom.data[i].gid+"' onclick='getgoodsdetails(this)'>\n" +
                "\t\t\t\t\t\t\t\t\t\t\t<div class=\"figure-img\">\n" +
                "\t\t\t\t\t\t\t\t\t\t\t\t<img src=\""+dom.data[i].img+"\" alt=\"\" />\n" +
                "\t\t\t\t\t\t\t\t\t\t\t</div>\n" +
                "\t\t\t\t\t\t\t\t\t\t\t<h3 class=\"title1\">"+dom.data[i].gname+"</h3>\n" +
                "\t\t\t\t\t\t\t\t\t\t\t<p class=\"desc\">"+dom.data[i].gdescribe+"</p>\n" +
                "\t\t\t\t\t\t\t\t\t\t\t<p class=\"price\">\n" +
                "\t\t\t\t\t\t\t\t\t\t\t\t<span class=\"num\">\n" +
                "\t\t\t\t\t\t\t\t\t\t\t\t\t"+dom.data[i].gprive+"\n" +
                "\t\t\t\t\t\t\t\t\t\t\t\t</span>\n" +
...
//点击更换视频
function addvido(dom) {
    //获取点击对应视频路径
    var str=$(dom);
    var video=str.attr("video");
...
function zhuyeshow() {
    ...
    //获取用户输入
    var uservague=$(".search-text").val()

    //存入服务器
    $.post("/goods.do?method=UserByVagueGoods",{vague:uservague},function(dosp) {
        //回写则代表保存成功
        //跳转界面
        console.log(dosp)
        if(dosp.code==200){
            location.href="/html/frontpage/shouwgoods.html"
        }else{
            alert("QAQ没有找到这个商品哦")
        }
...
//用户点击商品跳转详情页
function getgoodsdetails(dox){

    var abox=$(dox)

    var gids=abox.attr("gid")

    $.post("/goods.do?method=userGetByGoodsId",{gid:gids},function (goodsid) {

        //转成JSON字符串
        var goods=JSON.stringify(goodsid.data)
        //转回
        //JSON.parse()


        //存入session
         sessionStorage.setItem("goodsid",goods)

        //跳转界面
        location.href="/html/frontpage/goodsdetails.html"
...
//提示登录
function activation(dom) {
    alert("先去登录呀 你有个锤子的个人中心 嗯？")
}
